...
def group_by_centroid(restaurants, centroids):
    ...
    for i in restaurants:
        ...
        i_centroid = find_closest(i_location, centroids)
        lst.append([i_centroid, i])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    diff_x = [x_i - mean(xs) for x_i in xs]
    diff_y = [y_i - mean(ys) for y_i in ys]

    S_xx = sum([x ** 2 for x in diff_x])
    S_yy = sum([y ** 2 for y in diff_y])
    S_xy = sum([x * y for x, y in zip(diff_x, diff_y)])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for i in restaurants:
        if i in reviewed:
            d[restaurant_name(i)] = user_rating(user, restaurant_name(i)) 
        else:
            d[restaurant_name(i)] = predictor(i)
    return d
    # END Question 9
