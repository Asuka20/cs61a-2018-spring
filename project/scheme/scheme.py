...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("expected {0} argument(s) but got {1}".format(len(formals), len(vals)))
        while formals != nil:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("wrong number of arguments for {0}".format(str(self.fn)))
        # END PROBLEM 4
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    body = expressions.second
    procedure = MuProcedure(formals, body)
    return procedure
    # END PROBLEM 16
