...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.legitimacy:
                return
            # END Problem 13
...
# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt provides nothing but a High Wall."""
    name = 'Wall'
    implemented = True
    food_cost = 4
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is the watersafe version of ThrowerAnt. """
    implemented = True
    food_cost = 6
    watersafe = True
# END Problem 12
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        self.legitimacy = QueenAnt.legitimacy
        QueenAnt.legitimacy = False
        self.guardians = []
        # END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if not self.legitimacy:
            self.reduce_armor(self.armor)
        else:
            p = self.place.exit
            while p:
                if p.ant:
                    if isinstance(p.ant, BodyguardAnt) and p.ant.ant and p.ant.ant not in self.guardians:
                            p.ant.ant.damage *= 2
                            self.guardians.append(p.ant.ant)
                    if p.ant not in self.guardians:
                        p.ant.damage *= 2
                        self.guardians.append(p.ant)
                p = p.exit
