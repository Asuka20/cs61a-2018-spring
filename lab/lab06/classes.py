...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        if type(person) != str:
            ...
            print('{0} says: {1}'.format(person.name, person.talk()))
    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            thing = self.place.take(thing)
            self.backpack.append(thing)
            print('{0} takes the {1}'.format(self.name, thing.name))
    ...
    def unlock(self, place):
        ...
        if not key and place.locked:
            print("FSM can't be unlocked without a key!")
        elif key and place.locked:
            place.locked = False
            print("FSM is now unlocked!")
        elif key and not place.locked:
            print("FSM is already unlocked!")
